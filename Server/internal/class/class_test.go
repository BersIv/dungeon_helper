...
func TestGetAllClassesRepositoryError(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
	}
...
func (m *MockRepoTimeout) GetAllClasses(ctx context.Context) ([]Class, error) {
	select {
...
func TestServiceTimeout(t *testing.T) {
	mockRepo := &MockRepoTimeout{}
	service := &service{Repository: mockRepo, timeout: 3 * time.Second}

	ctx := context.Background()
	_, err := service.GetAllClasses(ctx)
	if err == nil {
		t.Error("expected an error, got nil")
	} else if err.Error() != "timeout exceeded" {
...
func TestGetAllClassesHandler(t *testing.T) {
	...
	fakeTokenGetter := &util.MockTokenGetter{Id: 123, Err: nil}
...
func TestGetAllClassesHandler_Unauthorized(t *testing.T) {
	...
	svc := &MockClassService{}
	fakeTokenGetter := &util.MockTokenGetter{Id: 0, Err: errors.New("authorization header is missing")}
	handler := NewHandler(svc, fakeTokenGetter)
...
func TestGetAllClassesHandler_ServiceError(t *testing.T) {
	...
	fakeTokenGetter := &util.MockTokenGetter{}
	handler := NewHandler(svc, fakeTokenGetter)
